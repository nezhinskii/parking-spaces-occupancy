...
class ParkingOccupancy:
    ...
    def __place_busy(sself, place, bb):
        return place.contains_point(bb[0] + (bb[2] - bb[0]) / 2, bb[1] + (bb[3] - bb[1]) * 3/4)
    
    def process_frame(self):
        results = self.model.predict(self.image_source, conf = self.CONFIDENCE_TRESHOLD)[0].boxes.xyxy
        result_mask = np.zeros((self.image_source.shape[0], self.image_source.shape[1], 3), dtype=np.uint8)
        for place in self.parking_places:
            flag = False
            for bb in results:
                if self.__place_busy(place, bb):
                    flag = True
                    break
            if flag:
